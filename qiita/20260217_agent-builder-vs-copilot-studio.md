...
本記事では、公式ドキュメント（[Choose between Microsoft 365 Copilot and Copilot Studio to build your agent](https://learn.microsoft.com/microsoft-365-copilot/extensibility/copilot-studio-experience)）をもとに、両者の違いと使い分けのポイントを整理します。
...
自然言語でインストラクションを記述し、SharePoint サイトや Microsoft Graph コネクタのデータをナレッジとして指定するだけで、コード不要でエージェントを構築できます。
...
- ライフサイクル管理（バージョン管理、段階的デプロイ、ロールバック）
- 利用状況の監視・分析ダッシュボード
...
| Copilot クレジット / 従量課金制 | ✅ 利用可能 | ✅ 利用可能 |
| 無料（Web ナレッジのみ） | ✅ 利用可能 | ❌ |

Microsoft 365 Copilot ライセンスがあれば、どちらのツールも追加費用なしで利用できます。ライセンスがない場合でも、Copilot クレジットまたは従量課金制プランでアクセスが可能です。
